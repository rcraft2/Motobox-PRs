...
public class WrapperPlayer extends WrapperEntity implements IWrapperPlayer {
    ...
    public void displayChatMessage(LanguageEntry language, Object... args) {
        Minecraft.getInstance().gui.getChat().addMessage(Component.literal(String.format(language.getCurrentValue(), args)));
    }
     ...
     * Called from WrapperEntity.onIVWorldUnload since NeoForge 1.21 doesn't allow @SubscribeEvent
     * in a superclass when the subclass is also registered as an event subscriber.
     */
    static void clearWrappersForLevel(LevelEvent.Unload event) {
