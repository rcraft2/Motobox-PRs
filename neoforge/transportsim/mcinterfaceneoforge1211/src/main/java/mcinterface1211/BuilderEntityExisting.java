    ...
    public void baseTick() {
        ...
        if (entity != null) {
            ...
            if (!entity.isValid) {
                ...
                // Shop respawn vehicles should never remain visibly on fire once restored.
                if (entity instanceof EntityVehicleF_Physics) {
                    EntityVehicleF_Physics vehicle = (EntityVehicleF_Physics) entity;
                    if (!vehicle.outOfHealth && vehicle.ownerUUID == null && vehicle.customPurchasePrice > 0) {
                        if (vehicle.damageVar.currentValue != 0) {
                            vehicle.damageVar.setTo(0.0D, !level().isClientSide);
                        }
                        vehicle.outOfHealth = false;
                        vehicle.ticksOutOfHealth = 0;
                        for (APart part : vehicle.allParts) {
                            if (part.damageVar.currentValue != 0) {
                                part.damageVar.setTo(0.0D, !level().isClientSide);
                            }
