...
public class BuilderEntityLinkedSeat extends ABuilderEntityBase {
    ...
    public boolean canAddPassenger(Entity passenger) {
        if (entity instanceof PartSeat && ((PartSeat) entity).placementDefinition.isController && passenger instanceof net.minecraft.world.entity.player.Player) {
            if (((PartSeat) entity).vehicleOn != null) {
                minecrafttransportsimulator.mcinterface.IWrapperPlayer player = WrapperPlayer.getWrapperFor((net.minecraft.world.entity.player.Player) passenger);
                if (!((PartSeat) entity).vehicleOn.canPlayerDrive(player)) {
                    if (((PartSeat) entity).vehicleOn.ownerUUID == null) {
                        player.sendPacket(new minecrafttransportsimulator.packets.instances.PacketPlayerChatMessage(player, minecrafttransportsimulator.systems.LanguageSystem.INTERACT_VEHICLE_UNOWNED, ((PartSeat) entity).vehicleOn.getVehicleUUIDString()));
                    } else {
                        player.sendPacket(new minecrafttransportsimulator.packets.instances.PacketPlayerChatMessage(player, minecrafttransportsimulator.systems.LanguageSystem.INTERACT_VEHICLE_NOTOWNER, ((PartSeat) entity).vehicleOn.getVehicleUUIDString()));
                    }
                    return false;
                }
            }
