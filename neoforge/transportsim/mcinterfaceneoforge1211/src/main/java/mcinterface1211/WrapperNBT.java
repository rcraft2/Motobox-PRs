...
class WrapperNBT implements IWrapperNBT {
    ...
    public List<IWrapperItemStack> getStacks(int count) {
        ...
        ContainerHelper.loadAllItems(tag, mcList, getRegistryAccess());
    ...
    public void setStacks(List<IWrapperItemStack> stacks) {
        ...
        ContainerHelper.saveAllItems(tag, mcList, getRegistryAccess());
    }
    ...
    private static HolderLookup.Provider getRegistryAccess() {
        ...
        //Fallback for client-side when server is not available
        try {
