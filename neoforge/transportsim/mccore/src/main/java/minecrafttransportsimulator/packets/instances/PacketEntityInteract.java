    ...
    public boolean handle(AWrapperWorld world, AEntityE_Interactable<?> entity, IWrapperPlayer player) {
        ...
        if (!leftClick && hitBox.definition != null && hitBox.definition.action != null) {
            if (vehicle != null && vehicle.lockedVar.isActive) {
                //Locked vehicles normally block action hitboxes, but unowned buyable vehicles should open purchase UI.
                if (rightClick && vehicle.ownerUUID == null && !vehicle.canPlayerDrive(player)) {
                    player.sendPacket(new PacketVehiclePurchaseGUIOpen(player, vehicle));
                } else if (rightClick) {
        ...
        if (rightClick) {
            if (entity instanceof PartSeat) {
                PartSeat seat = (PartSeat) entity;
                if (seat.vehicleOn != null && seat.placementDefinition.isController && !seat.vehicleOn.canPlayerDrive(player)) {
                    if (seat.vehicleOn.ownerUUID == null) {
                        // Open purchase GUI on the client that clicked.
                        player.sendPacket(new PacketVehiclePurchaseGUIOpen(player, (EntityVehicleF_Physics) seat.vehicleOn));
                    } else {
                        player.sendPacket(new PacketPlayerChatMessage(player, LanguageSystem.INTERACT_VEHICLE_NOTOWNER, seat.vehicleOn.getVehicleUUIDString()));
                    }
                    return false;
                }
            }
