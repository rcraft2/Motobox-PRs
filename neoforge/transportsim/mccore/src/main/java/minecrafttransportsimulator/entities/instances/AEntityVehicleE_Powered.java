...
public abstract class AEntityVehicleE_Powered extends AEntityVehicleD_Moving {
    ...
    public boolean canPlayerStartEngines(IWrapperPlayer player) {
        if (!canPlayerDrive(player)) {
            if (world.isClient()) {
                if (ownerUUID == null) {
                    player.displayChatMessage(LanguageSystem.INTERACT_VEHICLE_UNOWNED, uniqueUUID.toString());
                } else {
                    player.displayChatMessage(LanguageSystem.INTERACT_VEHICLE_NOTOWNER, uniqueUUID.toString());
                }
            }
            return false;
        }
        ...
        if (!ConfigSystem.settings.general.keyRequiredToStartVehicles.value) {
            ...
            // Some purchased respawns may not have a bound key yet.  If this driver is authorized,
            // allow engine start until they choose to bind a key.
            if (keyUUID == null) {
                return true;
            }
