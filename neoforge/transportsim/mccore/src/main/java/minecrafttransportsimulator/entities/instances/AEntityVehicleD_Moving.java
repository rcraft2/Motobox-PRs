...
abstract class AEntityVehicleD_Moving extends AEntityVehicleC_Colliding {
    ...
    public boolean theftVehicle;
    public double customPurchasePrice;
    private final Set<UUID> additionalDriverUUIDs = new HashSet<>();

    private static final String OWNER_UUID_TAG = "ownerUUID";
    private static final String THEFT_VEHICLE_TAG = "theftVehicle";
    private static final String CUSTOM_PURCHASE_PRICE_TAG = "customPurchasePrice";
    private static final String AUTHORIZED_DRIVER_UUIDS_TAG = "authorizedDrivers";
    ...
    public AEntityVehicleD_Moving(AWrapperWorld world, IWrapperPlayer placingPlayer, ItemVehicle item, IWrapperNBT data) {
        ...
        if (data != null) {
            ...
            this.ownerUUID = data.getUUID(OWNER_UUID_TAG);
            this.theftVehicle = data.getBoolean(THEFT_VEHICLE_TAG);
            this.customPurchasePrice = data.getDouble(CUSTOM_PURCHASE_PRICE_TAG);
            if (data.hasKey(AUTHORIZED_DRIVER_UUIDS_TAG)) {
                for (String driverUUIDText : data.getStrings(AUTHORIZED_DRIVER_UUIDS_TAG)) {
                    try {
                        additionalDriverUUIDs.add(UUID.fromString(driverUUIDText));
                    } catch (Exception ignored) {
                        // Ignore malformed UUIDs from legacy or manually edited saves.
                    }
                }
            }
    ...
    public boolean isDriverAuthorized(IWrapperPlayer player) {
        return player != null && isDriverAuthorized(player.getID());
    }

    public boolean isDriverAuthorized(UUID playerUUID) {
        if (playerUUID == null) {
            return false;
        }

        return ownerUUID != null && (playerUUID.equals(ownerUUID) || additionalDriverUUIDs.contains(playerUUID));
    }

    public boolean canPlayerDrive(IWrapperPlayer player) {
        return theftVehicle || isDriverAuthorized(player);
    }

    public boolean grantDriverAccess(UUID playerUUID) {
        if (playerUUID == null) {
            return false;
        }
        if (ownerUUID == null) {
            ownerUUID = playerUUID;
            additionalDriverUUIDs.add(playerUUID);
            lockedVar.setTo(0, true);
            return true;
        }
        return additionalDriverUUIDs.add(playerUUID);
    }

    public String getVehicleUUIDString() {
    ...
    public IWrapperNBT save(IWrapperNBT data) {
        ...
        if (ownerUUID != null) {
            data.setUUID(OWNER_UUID_TAG, ownerUUID);
        }
        data.setBoolean(THEFT_VEHICLE_TAG, theftVehicle);
        if (customPurchasePrice > 0) {
            data.setDouble(CUSTOM_PURCHASE_PRICE_TAG, customPurchasePrice);
        }
        List<String> driverUUIDs = new ArrayList<>();
        for (UUID driverUUID : additionalDriverUUIDs) {
            driverUUIDs.add(driverUUID.toString());
        }
        data.setStrings(AUTHORIZED_DRIVER_UUIDS_TAG, driverUUIDs);
