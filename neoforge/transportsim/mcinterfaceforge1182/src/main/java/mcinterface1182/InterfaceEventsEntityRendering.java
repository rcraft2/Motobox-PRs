...
import mcinterface1182.mixin.client.CameraMixin;
...
public class InterfaceEventsEntityRendering {
    ...
    public static void onIVPreLayer(PreLayer event) {
        ...
        if ((event.getOverlay() == ForgeIngameGui.HOTBAR_ELEMENT || event.getOverlay() == ForgeIngameGui.FOOD_LEVEL_ELEMENT || event.getOverlay() == ForgeIngameGui.PLAYER_HEALTH_ELEMENT || event.getOverlay() == ForgeIngameGui.ARMOR_LEVEL_ELEMENT || event.getOverlay() == ForgeIngameGui.EXPERIENCE_BAR_ELEMENT) && (InterfaceManager.clientInterface.getCameraMode() == CameraMode.FIRST_PERSON ? ConfigSystem.client.renderingSettings.renderHUD_1P.value : ConfigSystem.client.renderingSettings.renderHUD_3P.value)) {
