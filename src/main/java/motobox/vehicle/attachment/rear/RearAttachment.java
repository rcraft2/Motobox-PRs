...
public abstract class RearAttachment extends BaseAttachment<RearAttachmentType<?>> {
    ...
    public final Vec3d origin() {
        return this.vehicle.getTailPos();
    }
    ...
    public void setYaw(float yaw) {
        float diff = MathHelper.wrapDegrees(yaw - this.vehicle.getYaw());
        if (diff < -90 && diff > -180) yaw = this.vehicle.getYaw() - 90;
        else if (diff > 90 && diff < 180) yaw = this.vehicle.getYaw() + 90;

        this.yaw = yaw;
    }

    protected final void updateTrackedYaw(float yaw) {
        this.vehicle.setTrackedRearAttachmentYaw(yaw);
    }
